...
namespace KeepMeAlive.Helpers
{
    ...
    public static class VFX_UI
    {
        ...
        public static CustomTimer TransitPanel(ColorSpec color, Position pos, string label, float seconds = 0f)
        {
            ...
            var ui = gameUI?.LocationTransitTimerPanel;
            
            if (ui == null || gameUI == null || !gameUI.gameObject.activeInHierarchy)
            {
                return null;
            }
        ...
        public static CustomTimer ObjectivePanel(ColorSpec color, Position pos, string label, float seconds = 0f)
        {
            ...
            try
            {
                ...
                if (s_extractionPanel == null || !gameUI.gameObject.activeInHierarchy)
                {
                    return null;
                }
        ...
        // Best-effort pre-initialization used during raid load to avoid first-use races.
        public static bool TryWarmupPanels()
        {
            try
            {
                ...
                var transit = gameUI.LocationTransitTimerPanel;
                if (transit != null)
                {
                    if (!IsUnityAlive(s_transitRect))
                    {
                        s_transitRect = transit.transform.childCount > 0 ? transit.transform.GetChild(0) as RectTransform : null;
                    }

                    if (!IsUnityAlive(s_transitImage) && IsUnityAlive(s_transitRect))
                    {
                        s_transitImage = s_transitRect.GetComponent<Image>();
                    }
                    ...
                    if (!IsUnityAlive(s_transitText))
                    {
                        s_transitText = transit.GetComponentInChildren<TextMeshProUGUI>();
                    }
                }

                if (!IsUnityAlive(s_extractionPanel))
                {
                    s_extractionPanel = gameUI.BattleUiPanelExtraction;
                }

                if (IsUnityAlive(s_extractionPanel) && !IsUnityAlive(s_extractionRect))
                {
                    s_extractionRect = s_extractionPanel.transform.childCount > 0 ? s_extractionPanel.transform.GetChild(0) as RectTransform : null;
                }

                if (!IsUnityAlive(s_extractionImage) && IsUnityAlive(s_extractionRect))
                {
                    s_extractionImage = s_extractionRect.GetComponent<Image>();
                }

                if (!IsUnityAlive(s_extractionText) && IsUnityAlive(s_extractionRect))
                {
                    s_extractionText = s_extractionRect.GetComponentInChildren<TextMeshProUGUI>();
                }

                bool transitReady = transit != null && IsUnityAlive(s_transitRect);
                bool extractionReady = IsUnityAlive(s_extractionPanel) && IsUnityAlive(s_extractionRect);
                return transitReady && extractionReady;
            }
            catch (Exception ex)
            {
                Plugin.LogSource.LogWarning($"[VFX_UI.TryWarmupPanels] {ex.Message}");
                return false;
            }
        }
