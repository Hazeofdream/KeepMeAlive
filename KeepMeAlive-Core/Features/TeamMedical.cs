...
namespace KeepMeAlive.Features
{
    ...
    {
        //====================[ Constants & Fields ]====================
        private static float HEAL_HOLD_TIME => SyncedGameplayValues.TEAM_HEAL_HOLD_TIME;
        ...
        public static IEnumerable<Item> GetUsableMeds(Player healer, Player patient)
        {
            ...
            try
            {
                ...
                foreach (var item in healer.Inventory.AllRealPlayerItems)
                {
                    ...
                    // Always skip truly exhausted medkits (no charges left for any category).
        ...
        public static List<MedCategory> ClassifyMed(Item item)
        {
            var categories = new List<MedCategory>();
            try
            {
                if (item?.Template is not IHealthEffect template) return categories;
                ...
                if (template.DamageEffects != null)
                {
                    ...
                    if (template.DamageEffects.ContainsKey(EDamageEffectType.Intoxication) ||
                        template.DamageEffects.ContainsKey(EDamageEffectType.LethalIntoxication))
                        categories.Add(MedCategory.Comfort);
                }

                // Items with an HP resource pool (medkits) restore health.
                if (item.Template is MedsTemplateClass medsTemplate && medsTemplate.MaxHpResource > 0)
                {
                ...
                // Food/drinks that restore Energy / Hydration are Nutrition only.
                if (template.HealthEffects != null)
                {
                    if (template.HealthEffects.ContainsKey(EHealthFactorType.Poisoning) &&
                        !categories.Contains(MedCategory.Comfort))
                    {
                        categories.Add(MedCategory.Comfort);
                    }
                    ...
                    {
                        if (item is FoodDrinkItemClass && !categories.Contains(MedCategory.Nutrition))
                            categories.Add(MedCategory.Nutrition);
                    }
                }
            }
            ...
            Player healer, Player patient, MedCategory? category)
        {
            ...
            float minHp = SyncedGameplayValues.TEAM_HEAL_MIN_HP_RESOURCE;
            ...
            foreach (var med in GetUsableMeds(healer, patient))
            {
                ...
                {
                    if (med.Template is IHealthEffect template && template.DamageEffects != null)
                    {
        ...
        public static bool HealerHasMedForCategory(Player healer, MedCategory category)
        {
            try
            {
                ...
                foreach (var item in healer.Inventory.AllRealPlayerItems)
                {
                    if (item is not MedsItemClass && item is not FoodDrinkItemClass) continue;
                    var kit = item.GetItemComponent<MedKitComponent>();
                    if (kit != null)
                    {
                    ...
                    if (ClassifyMed(item).Contains(category)) return true;
                }
            }
            ...
            try
            {
                ...
                switch (category)
                {
                    ...
                    case MedCategory.Nutrition:
                        ...
                        float nutritionMinDeficit = Mathf.Max(0f, SyncedGameplayValues.TEAM_HEAL_NUTRITION_MIN_DEFICIT);
                        if (hc.FindActiveEffect<GInterface343>(EBodyPart.Common) != null) return true;
                        if (hc.FindActiveEffect<GInterface344>(EBodyPart.Common) != null) return true;
        ...
        internal class HealCompleteHandler
        {
            ...
            public void Complete(bool result)
            {
                ...
                onComplete?.Invoke(true); // Success, close the UI.
            }
        }
    }
}